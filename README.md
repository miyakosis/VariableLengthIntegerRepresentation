...
### 数値表現の比較 
||1バイトで表現できる範囲|2バイトで表現できる範囲|3バイトで表現できる範囲|...|
|:---|:---|:---|:---|:---|
|符号なし可変長整数|0 ～ 127|0 ～ 16383|0 ～ 2097151|...|
|符号つき可変長整数|-64 ～ 63|-8192 ～ 8191|-1048576 ～ 1048575|....|
|符号つき0近傍最適化可変長整数|-107 ～ 107|-1024 ～ 1023|-1048576 ～ 1048575|....|
...
BigEndian で可変長の複数バイトにて正の数値を表現します。  
...
BigEndian で可変長の複数バイトにて数値を表現します。  
...
##### 符号バイトについて
+64 と -64 は 終端バイトの表現が同じ(0100 0000)になります。  
先頭バイトの MSB+1 のビットが符号ビットとなるため、[0100 0000] は単体では負数として判断されます。  
そのため +64 を表現するためには符号ビットを保持するための [1000 0000] という符号バイトが必要となります。-128 と 0 の関係も同様です。  
...
しかし10進数での取り扱いを考えると、1バイトで 100 までの数を扱える方が利便性が高そうです。  
そこで UTF-8 エンコーディングのように、先頭バイトが-100～100であればその値、それ以外であれば可変長整数であるとする数値表現を検討しました。  
(2バイト、3バイトでの数値表現範囲を検討した結果、1バイトで 107～-107 が表現できます。)  
...
これにより、nバイト数で表現できる数値の範囲は以下の通りになります。  
1バイト：-107 ～ 107  
2バイト：-1024 ～ 1023  
...
たとえば 200 [0000 0000, 1100 1000] は、数値の1バイト目が 0x00 ですので、符号つき0近傍最適化可変長整数表現の1バイト目は 108 [0110 1100]、2バイト目は数値の2バイト目[1100 1000]をそのまま保持します。そのため符号つき0近傍最適化可変長整数表現では [0110 1100, 1100 1000]となります。   
逆変換の場合は、可変長整数表現の1バイト目が 108 [0110 1100]であることから2バイトで数値を表現していること、数値の1バイト目が 0x00 であることが分かります。  
同様に 456 [0000 0001, 1100 1000] は符号つき0近傍最適化可変長整数表現で表すと [0110 1101, 1100 1000]となります。  

(3バイトによる数値表現も同様に、可変長数値表現の1バイト目が数値の1バイト目により決定し、2バイト目、3バイト目に数値の2バイト目、3バイト目の値が設定されます)  
...
1048576(0001 0000, 0000 0000, 0000 0000) <-> [1000 0000, 1100 0000, 1000 0000, 0000 0000]	(※先頭バイト：-128)  
...
-1048577(1110 1111, 1111 1111, 1111 1111) <-> [1000 0000, 1111 1111, 1011 1111, 1111 1111, 0111 1111]	(※先頭バイト：-128)  


##### 冗長表現について  
たとえば 100 は [0110 0100] という1バイト表現の他、[0110 1100, 0110 0100] (※先頭バイト：108) という2バイトや [1000 0000, 1000 0000, 0110 0100] (※先頭バイト：-128) という3バイトでの冗長な表現も可能です。  
特に実用上大きな問題はないはずです。  
